                    ...
                    <table class="table table-hover m-0 text-center table-sm custom">
                      ...
                      <tbody>
                        ...
                        while ($data = mysqli_fetch_assoc($result)) {
                          ...
                          $dpl_nama = '';
                          // Prioritas 1: kolom dpl langsung (sama seperti yang dipakai di cetak)
                          if (!empty($data['dpl'])) {
                            $dpl_nama = $data['dpl'];
                          }
                          // Prioritas 2: fallback via dpl_pkl → dt_pegawai (setelah admin plotting)
                          if (empty($dpl_nama) && !empty($data['id_dpl'])) {
                            $q_dpl_pkl = mysqli_query($con, "SELECT nip FROM dpl_pkl WHERE id='$data[id_dpl]' LIMIT 1");
                            $d_dpl_pkl = mysqli_fetch_assoc($q_dpl_pkl);
                            if (!empty($d_dpl_pkl['nip'])) {
                              $q_peg = mysqli_query($con, "SELECT nama FROM dt_pegawai WHERE id='$d_dpl_pkl[nip]' LIMIT 1");
                        ...
                        ?>
                          <tr>
                            ...
                            <td class="text-center"><?php echo $data['nama_instansi']; ?></td>
                            <td class="text-center pr-1"><?php echo $dpl_nama ?: '<span class="text-muted">-</span>'; ?></td>
                            <td class="text-center pr-1"><?php echo $dcek['nm']; ?></td>
                            <td width="4%" class="text-center pl-1"><a class='btn btn-outline-info btn-xs btn-block' title='Lihat Detail & Upload Laporan' href='detailRiwayatPendaftaranPklUser.php?id=<?php echo $data['id']; ?>'><i class='far fa-folder-open'></i></a></td>
                            <td width="4%" class="text-center"><?php if ($data['val_adm'] == 1 or $data['val_adm'] == 4) {
                                                                  echo "<a class='btn btn-outline-danger btn-xs btn-block' onclick='return confirm(\"Yakin data ini dihapus?\")' title='Yakin data ini dihapus?' href='deletePendaftaranPklUser.php?id=" . $data['id'] . "'><i class='far fa-trash-alt'></i></a>";
                                                                } else {
                                                                  echo "<a class='btn btn-outline-secondary btn-xs btn-block' onclick='return confirm(\"Tidak bisa dihapus. Pendaftaran telah diterima\")' title='Tidak bisa dihapus. Pendaftaran telah diterima' disabled><i class='far fa-trash-alt'></i></a>";
                                                                } ?></td>
                            <td width="4%" class="text-center"><?php if (!empty($data['catatan'])) {
                                                                  echo "<a class='btn btn-outline-primary btn-xs btn-block' title='Lihat catatan' href='catatanPendaftaranPklUser.php?id=" . $data['id'] . "'><i class='far fa-comment-dots'></i></a>";
                                                                } else {
                                                                  echo "<a class='btn btn-outline-secondary btn-xs btn-block' onclick='return confirm(\"Tidak ada catatan.\")' title='Tidak ada catatan.' disabled><i class='far fa-comment-dots'></i></a>";
                                                                } ?></td>
