...
$upload_error = $_FILES['file_transkrip_nilai']['error'] ?? UPLOAD_ERR_NO_FILE;
...
$file_size = $_FILES['file_transkrip_nilai']['size'] ?? 0;
...
if (!move_uploaded_file($_FILES['file_transkrip_nilai']['tmp_name'], $namaftpd . '/' . $nama_file_transkrip_nilai)) {
    header("location:editPendaftaranKompreUserDua.php?id=$id&message=notifGagalUpload");
    exit;
}
