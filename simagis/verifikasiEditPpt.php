                ...
                <div class="col-lg-12">
                  <div class="table-responsive">
                    <table class="table custom">
                      <span class="lead">Opsi dan Profil Dosen Pembimbing Tesis</span><br>
                      <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modalJurnalDosen" style="margin-top:10px;">
                        <i class="glyphicon glyphicon-book"></i> Rekam Jejak Dosen
                      </button>
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        ...
                        ?>
                          <tr>
                            ...
                            <td><?php if ($ddpt['kuota1'] - $jumlahData1 == 0) {
                                  echo '<code>Penuh</code>';
                                } else {
                                  echo $ddpt['kuota1'] . ' (tersisa ' . $sisa1 . ')';
                                } ?></td>
                            <td><?php if ($ddpt['kuota2'] - $jumlahData2 == 0) {
                                  echo '<code>Penuh</code>';
                                } else {
                                  echo $ddpt['kuota2'] . ' (tersisa ' . $sisa2 . ')';
                                } ?></td>
                            <td class="text-center">
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header bg-primary" style="background-color: #337ab7; color: white;">
            ...
            <h4 class="modal-title"><i class="glyphicon glyphicon-book"></i> Rekam Jejak Dosen (Publikasi & Penelitian)</h4>
          </div>
          <div class="modal-body" style="padding:0;">
            <div style="padding: 10px; background-color: #f8f9fa; border-bottom: 1px solid #dee2e6;">
              <input type="text" id="searchJurnalDosen" class="form-control input-sm" placeholder="Cari nama dosen, NIP, penelitian, atau publikasi...">
            </div>
            <div style="padding: 10px; max-height: 70vh; overflow-y: auto;">
              <div class="panel-group" id="accordionJurnalDosen">
                <?php
                $csvFile = '../psychoApps/assets/jurnaldosen.csv';
                if (file_exists($csvFile)) {
                  ...
                  if ($handle !== FALSE) {
                    ...
                    while (($dataJurnal = fgetcsv($handle, 10000, ",")) !== FALSE) {
                      ...
                      $publikasi = '<ul style="padding-left:15px; margin-bottom:0; font-size:12px;">';
                      foreach ($publikasi_list as $pub) {
                        $publikasi .= '<li style="margin-bottom:4px; border-bottom:1px dashed #eee; padding-bottom:4px;">' . $pub . '</li>';
                      }
                      $publikasi .= '</ul>';

                      $penelitian_raw = htmlspecialchars($dataJurnal[3] ?? '-');
                      $penelitian_list = array_filter(array_map('trim', explode(';', $penelitian_raw)));
                      $penelitian = '<ul style="padding-left:15px; margin-bottom:0; font-size:12px;">';
                      foreach ($penelitian_list as $pen) {
                        $penelitian .= '<li style="margin-bottom:4px; border-bottom:1px dashed #eee; padding-bottom:4px;">' . $pen . '</li>';
                      }
                      $penelitian .= '</ul>';

                      echo '
                          <div class="panel panel-default jurnal-row" style="margin-bottom: 5px;">
                            <div class="panel-heading" role="tab" id="headingDosen' . $rowJurnal . '" style="padding:5px 10px;">
                              <h4 class="panel-title" style="font-size:14px;">
                                <a class="collapsed" role="button" data-toggle="collapse" data-parent="#accordionJurnalDosen" href="#collapseDosen' . $rowJurnal . '" aria-expanded="false" aria-controls="collapseDosen' . $rowJurnal . '" style="display:block; text-decoration:none;">
                                  <i class="glyphicon glyphicon-user" style="color:#337ab7;"></i> <strong style="color:#333;">' . $nama . '</strong> <small class="text-muted" style="margin-left:5px;">(NIP: ' . $nip . ')</small>
                                </a>
                              </h4>
                            </div>
                            <div id="collapseDosen' . $rowJurnal . '" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingDosen' . $rowJurnal . '">
                              <div class="panel-body" style="background-color: #f8f9fa; padding:10px;">
                                <div class="row">
                                  <div class="col-md-6" style="border-right: 1px solid #ddd;">
                                    <h5 class="text-info" style="font-weight:bold; border-bottom: 1px solid #ddd; padding-bottom: 5px; margin-top:0;"><i class="glyphicon glyphicon-book"></i> Publikasi</h5>
                                    ' . $publikasi . '
                                  </div>
                                  <div class="col-md-6">
                                    <h5 class="text-success" style="font-weight:bold; border-bottom: 1px solid #ddd; padding-bottom: 5px; margin-top:0;"><i class="glyphicon glyphicon-tint"></i> Penelitian</h5>
                                    ' . $penelitian . '
                                  </div>
                                </div>
                              </div>
                    ...
                    fclose($handle);
                    if ($rowJurnal == 1) {
                      echo "<div class='alert alert-warning text-center' style='margin:10px;'>Data kosong</div>";
                    }
                  } else {
                    echo "<div class='alert alert-danger text-center' style='margin:10px;'>Gagal membaca file CSV</div>";
                  }
                } else {
                  echo "<div class='alert alert-danger text-center' style='margin:10px;'>File CSV tidak ditemukan (../psychoApps/assets/jurnaldosen.csv)</div>";
                }
                ?>
              </div>
            </div>
