      ...
      <div class="row">
        ...
        <div class="panel panel-success">
          <div class="panel-heading">
            <ul class="list">
              <li>Berikut adalah data seluruh Pendaftar Ujian Tesis dari semua periode pendaftaran.</li>
            ...
            <div class="row">
              <div class="col-md-12" style="margin-bottom:10px;">
                <ul class="nav nav-tabs">
                  ...
                  <li role="presentation" class="active"><a>Semua Pendaftar</a></li>
                </ul>
              </div>
              <div class="col-md-12">
                  ...
                  <table class=" table table-condensed table-bordered table-striped custom" width="100%" style="margin-bottom:0px; font-size:13px;">
                    <thead>
                      <tr>
                        ...
                        <th rowspan="2" class="text-center" width="22%">Nama | NIM | Hp<br/>Tgl. Daftar | Periode</th>
                    ...
                    <tbody class="text-muted">
                      <?php
                        $no=0;
                        $sql =  "SELECT * FROM mag_peserta_ujtes ORDER BY id DESC";
                        ...
                        $rperiod = mysqli_query($GLOBALS["___mysqli_ston"], $qryperiod);
                        $dperiod = mysqli_fetch_assoc($rperiod);
                        $qry_thp = "SELECT * FROM mag_opsi_tahap_ujprop_ujtes WHERE id='$dperiod[tahap]'";
                        $hasilthp = mysqli_query($GLOBALS["___mysqli_ston"], $qry_thp);
                        $dthp = mysqli_fetch_assoc($hasilthp);
                        $qry_nm_ta = "SELECT * FROM mag_dt_ta WHERE id='$dperiod[ta]'";
                        $hasilta = mysqli_query($GLOBALS["___mysqli_ston"], $qry_nm_ta);
                        $dnta = mysqli_fetch_assoc($hasilta);
                        $qry_nm_smt = "SELECT * FROM opsi_nama_semester WHERE id='$dnta[semester]'";
                        $hsmt = mysqli_query($GLOBALS["___mysqli_ston"], $qry_nm_smt);
                        $dsemester = mysqli_fetch_assoc($hsmt);   
                        $periode_str = 'Thp '.$dthp['tahap'].' Smt '.$dsemester['nama'].' TA '.$dnta['ta'];
                        ?>
                      <tr>
                        <td class="text-center"><?php echo $no;?></td>
                        <td><?php echo $dmhssw['nama'].' | '.$dmhssw['nim'].' | '.$dmhssw['kntk'].'<br/>'.$data['tgl_pendaftaran'].' | <span class="label label-info">'.$periode_str.'</span>';?></td>
          ...
          <div class="modal-content">
            <div class="modal-header">
              ...
              <h4 class="modal-title" id="modalCrtJad">
                <?php $sql =  "SELECT * FROM mag_peserta_ujtes WHERE id = '".$_GET['id']."'"; // This is wrong for modal. Modal gets populated via AJAX. Actually this PHP snippet is evaluating at render time without knowing the id. It doesn't matter because it just prints Input Jadwal or Edit Jadwal based on $data['cek']. Let's just leave it as Input/Edit Jadwal.
                  echo "Input / Edit Jadwal"; ?>
              </h4>
            </div>
            <div class="modal-body">
