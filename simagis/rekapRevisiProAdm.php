...
<html lang="en">
  ...
  <body>
    ...
    $sql =  "SELECT mrs.id_sempro, MAX(mrs.id) AS max_id FROM mag_revisi_sempro mrs INNER JOIN mag_periode_pendaftaran_sempro mps ON mrs.id_sempro = mps.id INNER JOIN mag_dt_ta mta ON mps.ta = mta.id INNER JOIN mag_opsi_tahap_ujprop_ujtes moth ON mps.tahap = moth.id GROUP BY mrs.id_sempro ORDER BY max_id DESC";
